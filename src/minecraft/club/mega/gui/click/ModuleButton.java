...
public class ModuleButton {
    ...
    public final void drawButton(final int mouseX, final int mouseY) {
        if (components.isEmpty() && Mega.INSTANCE.getClickGUI().getCurrent() == 1 && this.isExpand())
            for (final Setting setting : module.getSettings()) {
                ...
                if(setting instanceof RangeSetting) {
                    components.add(new RangeComponent((RangeSetting) setting, width, 30));
                }
                if (setting instanceof NumberSetting)
                    components.add(new NumberComponent((NumberSetting) setting, width, height));
                if (setting instanceof ColorSetting)
                    components.add(new ColorComponent((ColorSetting) setting, width, height));
            }

        RenderUtil.drawRect(x, y, width, height, new Color(30, 30, 30, 255));
        if(MouseUtil.isInside(mouseX, mouseY, x, y, width, height)) {
            RenderUtil.drawRoundedRect(x + 3, y + 1, width - 6, height - 2,3, new Color(12, 12, 12, 255));
        }
        Mega.INSTANCE.getFontManager().getFont("Arial 21").drawString(module.getName(), x + 5, y + 5 - 1, module.isToggled() ? ClickGui.getInstance().color.getColor() : new Color(255, 255, 255));
        if (MouseUtil.isInside(mouseX, mouseY, x, y, width, height)) {
            //Mega.INSTANCE.getFontManager().getFont("Arial 19").drawStringWithShadow(module.getDescription(), mouseX + 5, mouseY + 5, new Color(255, 255, 255, 255));
        }
        if (!module.getSettings().isEmpty())
            Mega.INSTANCE.getFontManager().getFont("Arial 27").drawString(expand ? "-" : "+", x + width - 18, (expand ? y + 3 : y + 4) - 2, -1);
        ...
        for (final Component component : components) {
            ...
            if (prevComponent == null)
                component.setY(y +  height + (component instanceof RangeComponent ? 10:0) );
            else

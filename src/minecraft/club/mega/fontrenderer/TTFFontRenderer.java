...
public class TTFFontRenderer {
    ...
    private int renderString(String text, float x, float y, int color, boolean shadow) {
        if(text.contains(Minecraft.getMinecraft().session.getUsername()) && Mega.INSTANCE.getModuleManager().getModule("NameProtect").isToggled()) {
            text = text.replace(Minecraft.getMinecraft().session.getUsername(), Mega.INSTANCE.getUsername());

        }
        for(int i = 0; i < FriendUtil.getFriends().size(); i++) {
            if(text.contains((FriendUtil.getFriends().get(i)))) {
                text = text.replace(FriendUtil.getFriends().get(i), "§f[§aFriend§f]§r " + FriendUtil.getFriends().get(i));
            }
        }
